...
Научный руководитель — Владимир А. Пархоменко, старший преподаватель ВШПИ СПбПУ.
...
- **Версия B** (`ocean-static.html`) — статичная страница (контрольная группа)
- **Версия C** (`ocean-heading-parallax.html`) — параллакс только в заголовках разделов

Для сбора данных о движениях глаз применялась библиотека [WebGazer.js](https://webgazer.cs.brown.edu/). В эксперименте приняли участие 55 человек.
...
├── ocean-static.html            # Версия B: статика (контроль)
...
│   ├── raw/                     # Сырые JSON-файлы сессий (55 участников)
...
> **Важно:** стенд использует HTTPS (файлы `cert.pem` и `key.pem`), так как WebGazer.js требует защищённое соединение для доступа к веб-камере. При первом открытии браузер покажет предупреждение — нажмите «Продолжить» (или «Advanced → Proceed»).
